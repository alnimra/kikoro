...
import { KikoroLogo, KIKORO_LOGO_MASK_SVG } from "@/components/icons/kikoro-logo";
...
function WebLogoShimmer({ color }: { color: string }) {
  ...
  const shimmerStyle = useMemo(
    () => ({
      width: LOGO_SIZE,
      height: LOGO_SIZE,
      WebkitMaskImage: `url("data:image/svg+xml,${encodeURIComponent(KIKORO_LOGO_MASK_SVG)}")`,
  ...
  const maskElement = useMemo(
    () => (
      <View style={styles.shimmerMask}>
        <KikoroLogo size={LOGO_SIZE} color="#000000" />
      </View>
    ),
    [],
  );
    ...
    <MaskedView style={trackStyle} maskElement={maskElement}>
      <View style={trackStyle}>
        <View style={styles.nativeShimmerBase}>
          <KikoroLogo size={LOGO_SIZE} color={color} />
        </View>
